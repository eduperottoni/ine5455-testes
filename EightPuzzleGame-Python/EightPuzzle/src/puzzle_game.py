...
class PuzzleGame:
    ...
    def __put_tiles_in_dic_positions__(self, list_tiles):
        ...
        for line in range(
            ...
            for column in range(1, self.dimension + 1):
                self.dic_positions_of_tiles.update(
                    {next(iterator_tiles): (line, column)}
                )
        for column in range(1, self.dimension):  # last line
            self.dic_positions_of_tiles.update(
                {next(iterator_tiles): (self.dimension, column)}
            )
    ...
    def move_tile_from_a_position_to_the_empty_position(self, line, column):
        if self.board.is_inside_the_board(
            line, column
        ) and self.board.positions_are_adjacent(
            line, column, self.line_of_empty_position, self.column_of_empty_position
        ):
    ...
    def move_tile(self, tile):
        ...
        if self.board.is_inside_the_board(
            ...
            tile_line,
            tile_column,
            self.line_of_empty_position,
            self.column_of_empty_position,
        ):
            self.board.put_tile(
                tile, self.line_of_empty_position, self.column_of_empty_position
            )
    ...
    def __move_empty_cell_to_down__(self):
        if self.board.is_in_the_bottom_border(
            self.line_of_empty_position, self.column_of_empty_position
        ):
            return False
        else:
            ...
            self.board.change_tiles_in_positions(
                self.line_of_empty_position,
                self.column_of_empty_position,
                new_empty_line,
                new_empty_column,
    ...
    def __move_empty_cell_to_up__(self):
        if self.board.is_in_the_superior_border(
            self.line_of_empty_position, self.column_of_empty_position
        ):
            return False
        else:
    ...
    def __move_empty_cell_to_right__(self):
        if self.board.is_in_the_right_border(
            self.line_of_empty_position, self.column_of_empty_position
        ):
            return False
        else:
    ...
    def __move_empty_cell_to_left__(self):
        if self.board.is_in_the_left_border(
            self.line_of_empty_position, self.column_of_empty_position
        ):
            return False
        else:
    ...
    def get_tile(self, line, column):
        ...
        if (
            ...
            print("3")
            if (
                line == self.line_of_empty_position
                and column == self.column_of_empty_position
            ):
                print("4")
                return " "
            else:

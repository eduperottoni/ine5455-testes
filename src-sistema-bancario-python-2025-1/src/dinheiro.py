...
class Dinheiro:
    ...
    def obter_quantia_em_escala(self):
        return abs(self.__inteiro) * self.__moeda.base_fracionaria() + abs(
            self.__fracionado
        )
    ...
    def formatar_com_moeda(self):
        return "{0},{1:02d} {2}".format(
            int(self.inteiro), int(self.fracionado), self.moeda.name
        )
    ...
    def __eq__(self, other):
        if isinstance(other, Dinheiro):
            ...
            mesmo_valor = (self.inteiro == other.inteiro) and (
                self.fracionado == other.fracionado
            )
...
class ValorMonetario:
    ...
    def somar(self, quantia_somada):
        ...
        valor = (
            self.__quantia.obter_quantia_em_escala() * self.__sinal
            + quantia_somada.obter_quantia_em_escala()
        )
    ...
    def subtrair(self, quantia_subtraida):
        ...
        valor = (
            self.__quantia.obter_quantia_em_escala() * self.__sinal
            - quantia_subtraida.obter_quantia_em_escala()
        )
        return ValorMonetario(self.__moeda, valor)
    ...
    def __eq__(self, other):
        if isinstance(other, ValorMonetario):
            ...
            iguais_com_mesma_moeda = (
                self.__sinal.__eq__(other.__sinal)
                and self.__quantia.__eq__(other.__quantia)
                and self.__moeda.__eq__(other.__moeda)
            )
